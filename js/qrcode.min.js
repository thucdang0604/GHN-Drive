...
(function(root) {
  var QR = function(el, options) {
    if (typeof options === 'string') options = { text: options };
    options = options || {};
    var text = options.text || '';
    if (!text) return;
    // Dùng Level L (Low) để tối đa hóa dung lượng và cho mắt lưới thưa nhất, dễ quét nhất
    var level = (options.correctLevel && typeof options.correctLevel === 'string' && text.length < 500) ? options.correctLevel : 'L';
    var qr = qrcode(0, level);
    qr.addData(text);
    qr.make();
    if (el) {
      el.innerHTML = qr.createImgTag(options.cellSize || 3, options.margin || 4);
      if (el.querySelector) {
  ...
  QR.CorrectLevel = { L: 'L', M: 'M', Q: 'Q', H: 'H' };
  root.QRCode = QR;
